...
# Catch-all so an unexpected exception returns clean JSON instead of leaking a
# raw stack/message. Normal HTTP errors (401/404/etc.) pass through unchanged.
...
def rewrite_section():
    ...
    try:
        ...
        system_prompt = f"""You are a professional newsletter copywriter for the wedding planning industry.
...
- Use ONLY original, generic characters and scenes. NEVER name a real person,
  ...
  situation/emotion (e.g. "an exhausted planner buried in seating charts") instead.
...
def generate_meme():
    ...
    try:
        data = request.json
        month = data.get('month') or ''  # avoid .lower() crash when omitted
...
# NOTE: Removed the dead /api/save-newsletter-archive endpoint — it wrote to
# ephemeral container disk, was never read back, and had no frontend caller.
# The real archive is the GCS published/ store (publish-draft + list-published).
...
def search_all_signals(time_window: str = '30d', exclude_urls: list = None) -> list:
    ...
    print(f"[Insight Builder] Searching all 8 signals (US focus, in parallel)...")

    # Search all signals CONCURRENTLY (was sequential — 8x round-trips end to end).
...
Return results with title, url, publisher, published_date, and summary with key data points."""
    ...
    # Collect + dedup across signals (shared exclude means some overlap; dedup here).
